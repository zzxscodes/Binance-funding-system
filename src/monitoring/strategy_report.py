...
class StrategyReportGenerator:
    ...
    def _load_position_history(self) -> None:
        ...
        try:
            # 加载所有账户的历史数据文件
            for history_file in self.position_history_dir.glob('*_position_history.json'):
                account_id = history_file.stem.replace('_position_history', '')
                try:
                    with open(history_file, 'r', encoding='utf-8') as f:
                        data = json.load(f)
                        self._position_history[account_id] = data.get('records', [])
                    logger.debug(f"Loaded {len(self._position_history[account_id])} position records for {account_id}")
                except Exception as e:
                    logger.warning(f"Failed to load position history from {history_file}: {e}")
        except Exception as e:
    ...
    def _save_position_history(self, account_id: str) -> None:
        """保存持仓历史数据到文件"""
        try:
            history_file = self.position_history_dir / f"{account_id}_position_history.json"
            data = {
            ...
            logger.debug(f"Saved {len(data['records'])} position records for {account_id}")
        except Exception as e:
            logger.error(f"Failed to save position history for {account_id}: {e}", exc_info=True)
    ...
    def record_position_opening(
        ...
        try:
            ...
            logger.info(f"Recorded position opening for {account_id}: {symbol} {position_amt} @ {entry_price}")
    ...
    def _get_funding_rate_structure(
        ...
        try:
            ...
            funding_rates_map = self.data_api.get_funding_rate_between(begin_label, end_label)
            ...
            for symbol in symbols:
                if symbol not in funding_rates_map:
                    ...
                    logger.warning(f"No funding rate data found for {symbol} in time range {start_date.date()} to {end_date.date()}")
                    continue
    ...
    async def _calculate_strategy_wear(
        ...
        try:
            ...
            # 获取Equity Curve
            equity_df = self.equity_curve_tracker.get_equity_curve(account_id, start_date, end_date)
            ...
            # 从配置读取费率（如果未配置则使用默认值）
            fee_rate = config.get('strategy.report.fee_rate', 0.0004)  # 0.04% per trade (open + close)
            slippage_rate = config.get('strategy.report.slippage_rate', 0.0001)  # 0.01%
            
            # 从真实交易记录计算
            for record in position_openings:
                position_value = abs(record.get('position_amt', 0)) * record.get('entry_price', 0)
                if position_value > 0:
                    # 手续费：每次开仓/平仓双向
                ...
                if record.get('funding_rate') is not None:
                    ...
                    if position_value > 0:
                        ...
                        try:
                            ...
                            days_held = (end_date - record_time).total_seconds() / 86400.0
                            funding_periods = max(1, int(days_held * 3))  # 每天3次（每8小时）
                            funding_fees_actual += position_value * abs(record['funding_rate']) * funding_periods
                        except Exception as e:
                            logger.debug(f"Failed to calculate funding fees for record: {e}")
            ...
            if not equity_df.empty and 'equity' in equity_df.columns:
                ...
                initial_equity = float(account_info.get('totalWalletBalance', 0) or 
                                      account_info.get('totalMarginBalance', 0) or 0)
            else:
                initial_equity = 0.0
            
            wear_pct = (total_wear / initial_equity * 100.0) if initial_equity > 0 else 0.0
    ...
    async def generate_report(
        ...
        try:
            ...
            # 2. 获取持仓信息
            positions = await self.system_api.get_position_info(account=account_id)
            current_symbols = [p['system_symbol'] for p in positions if abs(float(p.get('position_amt', 0))) > 1e-8]
            
            # 3. 资金费率结构
            # 资金费率结构需要使用更长的历史数据（7天）来统计，而不是程序执行期间
            # 因为资金费率需要足够的历史数据才能计算平均值、标准差等统计指标
            funding_start_date = end_date - timedelta(days=7)  # 使用最近7天的数据
            funding_structure = self._get_funding_rate_structure(current_symbols, funding_start_date, end_date)
            ...
            logger.info(f"Filtered position openings: {len(position_openings)} out of {len(all_position_openings)} total (strategy execution period)")
            
            # 5. 策略磨损分析
            wear_analysis = await self._calculate_strategy_wear(account_id, start_date, end_date, position_openings)
            
            # 6. 获取账户信息用于metadata
            account_info = await self.system_api.get_account_info(account=account_id)
            initial_capital = 0.0
            if account_info:
                initial_capital = float(account_info.get('totalWalletBalance', 0) or 
                                      account_info.get('totalMarginBalance', 0) or 0)
            # 如果账户信息为空，尝试从Equity Curve获取
            if initial_capital <= 0 and not equity_df.empty and 'equity' in equity_df.columns:
            ...
            report = {
                ...
                'equity_curve': {
                    ...
                    'data': self._convert_dataframe_to_dict(equity_df) if not equity_df.empty else []
                },
                'coin_selection': {
    ...
    def _save_report(self, account_id: str, report: Dict, overwrite: bool = False) -> None:
        """保存报告到文件"""
        try:
            if overwrite:
                # 覆盖模式：每个账户只保留一个最新报告
                ...
                # 删除该账户的旧报告文件（如果有）
                for old_file in self.reports_dir.glob(f"{account_id}_strategy_report_*.json"):
        ...
        try:
            ...
            for calc in calculators:
                ...
                for key, value in calc_params.items():
                    ...
                    elif isinstance(value, (int, float)) and isinstance(merged_params[key], (int, float)):
                        # 数值类型取平均值
                        merged_params[key] = (merged_params[key] + value) / 2
                    # 其他类型保持第一个值
            
            # 添加策略方向（如果有多个calculator，取第一个）
            strategy_directions = [calc.get_description().get('strategy_direction') 
                                 for calc in calculators 
