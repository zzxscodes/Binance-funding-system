...
class MeanBuyDolvol4OverDolvolRankCalculator7(AlphaCalculatorBase):
    """
    买入金额占比排名因子。
    ...
    def __init__(self, lookback_bars: int = 1000, name: Optional[str] = None):
        ...
        self.name = name or f"mean_buy_dolvol4_over_dolvol_rank_7_{self.lookback_bars}"
...
# 创建默认实例（系统会自动发现）
CALCULATOR_INSTANCE = MeanBuyDolvol4OverDolvolRankCalculator7(lookback_bars=1000, name="mean_buy_dolvol4_over_dolvol_rank_7_1000")
