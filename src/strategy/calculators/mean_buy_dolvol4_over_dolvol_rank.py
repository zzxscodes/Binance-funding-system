...
class MeanBuyDolvol4OverDolvolRankCalculator(AlphaCalculatorBase):
    """
    买入金额占比排名因子。
    ...
    def __init__(self, lookback_bars: int = 1000, name: Optional[str] = None):
        ...
        self.name = name or f"mean_buy_dolvol4_over_dolvol_rank_{self.lookback_bars}"
...
# 创建默认实例（系统会自动发现）
CALCULATOR_INSTANCE = MeanBuyDolvol4OverDolvolRankCalculator(lookback_bars=1000, name="mean_buy_dolvol4_over_dolvol_rank_1000")
