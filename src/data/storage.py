...
class DataStorage:
    ...
    def __init__(self):
        ...
        self.funding_rates_dir = Path(config.get('data.funding_rates_directory', 'data/funding_rates'))
        self.premium_index_dir = Path(config.get('data.premium_index_directory', 'data/premium_index'))
    ...
    def _get_trade_file_path(self, symbol: str, date: datetime, hour: Optional[int] = None) -> Path:
        """
        获取逐笔成交数据文件路径
    ...
    def save_klines(self, symbol: str, df: Union[pd.DataFrame, pl.DataFrame], date: Optional[datetime] = None):
        """
            ...
            if date is None:
                if 'open_time' in df_pl.columns:
                    ...
                    if len(dates_in_data) > 1:
                        # 数据跨多天，需要分别保存
                        logger.debug(f"Kline data for {symbol} spans {len(dates_in_data)} days, saving to separate files")
                        for date_only in sorted(dates_in_data):
                            date_dt = datetime.combine(date_only, datetime.min.time()).replace(tzinfo=timezone.utc)
                            # 过滤出该日期的数据
                            day_df = df_pl.filter(pl.col('open_time').dt.date() == date_only)
    ...
    def _save_klines_to_file(self, symbol: str, df_pl: pl.DataFrame, file_path: Path):
        ...
        try:
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if existing_df_pl.is_empty():
                            logger.debug(f"Existing kline file is empty, will overwrite with new data: {file_path}")
                        else:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            logger.warning(f"Failed to merge existing file {file_path} after {max_retries} attempts: {e}, will overwrite")
                    except Exception as e:
                        logger.warning(f"Failed to merge existing file {file_path}: {e}, will overwrite")
                        break
            ...
            logger.error(f"Failed to save klines to file {file_path}: {e}", exc_info=True)
            raise
    
    def load_klines(self, symbol: str, start_date: Optional[datetime] = None,
        ...
        try:
            ...
            if start_date and end_date:
                ...
                while current_date <= end_date_only:
                    file_path = self._get_kline_file_path(symbol, 
            ...
            if len(dfs_pl) > 1:
                ...
                try:
                    ...
                    # 合并成功，更新缓存
                    if cached_schema is None or cached_schema.get('schema') != dfs_pl[0].schema:
                    ...
                    logger.debug(f"Schema mismatch detected for {symbol}, unifying schemas: {e}")
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in as_completed(fut_map):
                ...
                try:
                    ...
                    logger.error(f"Bulk load klines failed for {symbol}: {e}", exc_info=True)
                    result[format_symbol(symbol)] = pd.DataFrame()

        return result
    
    def save_trades(self, symbol: str, df: Union[pd.DataFrame, pl.DataFrame], date: Optional[datetime] = None):
        """
            ...
            for attempt in range(max_retries):
                try:
                    ...
                    if 'tradeId' in df.columns and 'tradeId' in existing_df_pl.columns:
                        ...
                        combined_df = combined_df.unique(subset=['tradeId'], keep='last')
                    ...
                    if attempt < max_retries - 1:
                        ...
                        logger.warning(f"Failed to merge existing trades file {file_path} after {max_retries} attempts: {e}")
                except Exception as e:
                    logger.warning(f"Failed to merge existing trades file {file_path}: {e}")
                    break
    ...
    def _load_dataframe_polars(self, file_path: Path) -> pl.DataFrame:
        ...
        try:
            ...
            # 如果是parquet文件损坏，尝试删除它以便重新生成
            if file_path.suffix == '.parquet' and 'out of specification' in str(e):
                try:
                    logger.warning(f"Corrupted parquet file detected, attempting to remove: {file_path}")
                    file_path.unlink()
                    logger.info(f"Removed corrupted parquet file: {file_path}")
                except Exception as del_err:
                    logger.error(f"Failed to remove corrupted file {file_path}: {del_err}")
            return pl.DataFrame()
    ...
    def save_funding_rates(self, symbol: str, df: Union[pd.DataFrame, pl.DataFrame], date: Optional[datetime] = None):
        """
        ...
        try:
            ...
            if date is None:
                ...
                if 'fundingTime' in df_pl.columns:
                    ...
                    if len(dates_in_data) > 1:
                        # 数据跨多天，需要分别保存
                        logger.debug(f"Funding rate data for {symbol} spans {len(dates_in_data)} days, saving to separate files")
                        for date_only in sorted(dates_in_data):
                            date_dt = datetime.combine(date_only, datetime.min.time()).replace(tzinfo=timezone.utc)
                            # 过滤出该日期的数据
                            day_df = df_pl.filter(pl.col('fundingTime').dt.date() == date_only)
                            if not day_df.is_empty():
                                file_path = self._get_funding_rate_file_path(symbol, date_dt)
                                self._save_funding_rates_to_file(symbol, day_df, file_path)
                        return  # 已按日期分组保存，直接返回
                    else:
                        # 单日数据，使用该日期
    ...
    def _save_funding_rates_to_file(self, symbol: str, df_pl: pl.DataFrame, file_path: Path):
        ...
        try:
            ...
            if file_path.exists():
                ...
                for attempt in range(max_retries):
                    try:
                        existing_df_pl = self._load_dataframe_polars(file_path)
                        if existing_df_pl.is_empty():
                            logger.debug(f"Existing funding rate file is empty, will overwrite with new data: {file_path}")
                        else:
                            combined_df = pl.concat([existing_df_pl, df_pl])
                            combined_df = combined_df.unique(subset=['fundingTime'], keep='last')
            ...
            # 保存
            self._save_dataframe_polars(df_pl, file_path)
            logger.debug(f"Saved {len(df_pl)} funding rates for {symbol} to {file_path}")
            
        except Exception as e:
            logger.error(f"Failed to save funding rates to file {file_path}: {e}", exc_info=True)
            raise
    
    def load_funding_rates(self, symbol: str, start_date: Optional[datetime] = None,
                          end_date: Optional[datetime] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if start_date and end_date:
                ...
                end_date_only = end_date.date()
                while current_date <= end_date_only:
                    file_path = self._get_funding_rate_file_path(symbol, 
                        datetime.combine(current_date, datetime.min.time()).replace(tzinfo=timezone.utc))
                    if file_path.exists():
                        all_files.append(file_path)
                    current_date += timedelta(days=1)
            ...
            logger.error(f"Failed to load funding rates for {symbol}: {e}", exc_info=True)
            return pd.DataFrame()
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            ...
            for fut in as_completed(fut_map):
                ...
                try:
                    ...
                    logger.error(f"Bulk load funding rates failed for {symbol}: {e}", exc_info=True)
                    result[format_symbol(symbol)] = pd.DataFrame()

        return result
    
    def save_premium_index_klines(self, symbol: str, df: Union[pd.DataFrame, pl.DataFrame], date: Optional[datetime] = None):
        """
        ...
        try:
            ...
            if date is None:
                if 'open_time' in df_pl.columns:
                    ...
                    if len(dates_in_data) > 1:
                        # 数据跨多天，需要分别保存
                        logger.debug(f"Premium index kline data for {symbol} spans {len(dates_in_data)} days, saving to separate files")
                        for date_only in sorted(dates_in_data):
                            date_dt = datetime.combine(date_only, datetime.min.time()).replace(tzinfo=timezone.utc)
                            # 过滤出该日期的数据
                            day_df = df_pl.filter(pl.col('open_time').dt.date() == date_only)
                            if not day_df.is_empty():
                                file_path = self._get_premium_index_file_path(symbol, date_dt)
                                self._save_premium_index_klines_to_file(symbol, day_df, file_path)
                        return  # 已按日期分组保存，直接返回
                    else:
                        # 单日数据，使用该日期
                        min_time = df_pl['open_time'].min()
    ...
    def _save_premium_index_klines_to_file(self, symbol: str, df_pl: pl.DataFrame, file_path: Path):
                ...
                for attempt in range(max_retries):
                    try:
                        ...
                        if attempt < max_retries - 1:
                            ...
                            logger.warning(f"Failed to merge existing file {file_path} after {max_retries} attempts: {e}")
                    except Exception as e:
                        logger.warning(f"Failed to merge existing file {file_path}: {e}")
                        break
            
            # 保存
            self._save_dataframe_polars(df_pl, file_path)
            logger.debug(f"Saved {len(df_pl)} premium index klines for {symbol} to {file_path}")
            
        except Exception as e:
            logger.error(f"Failed to save premium index klines to file {file_path}: {e}", exc_info=True)
            raise
    
    def load_premium_index_klines(self, symbol: str, start_date: Optional[datetime] = None,
                                  end_date: Optional[datetime] = None) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if start_date and end_date:
                ...
                end_date_only = end_date.date()
                while current_date <= end_date_only:
                    file_path = self._get_premium_index_file_path(symbol, 
                        datetime.combine(current_date, datetime.min.time()).replace(tzinfo=timezone.utc))
                    if file_path.exists():
                        all_files.append(file_path)
                    current_date += timedelta(days=1)
            ...
            logger.error(f"Failed to load premium index klines for {symbol}: {e}", exc_info=True)
            return pd.DataFrame()
    ...
    def load_premium_index_klines_bulk(
        ...
        with ThreadPoolExecutor(max_workers=workers) as ex:
            fut_map = {
                ex.submit(self.load_premium_index_klines, symbol, start_date, end_date): symbol
                for symbol in symbols
            }
            for fut in as_completed(fut_map):
                symbol = fut_map[fut]
                try:
                    ...
                    logger.error(f"Bulk load premium index klines failed for {symbol}: {e}", exc_info=True)
                    result[format_symbol(symbol)] = pd.DataFrame()
